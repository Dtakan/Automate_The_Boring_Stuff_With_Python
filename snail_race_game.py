# snail race game, which snail will win? 
...
print('''Snail Race Game by, @Dtakan
''')

# Ask how many snails to race
while True:  # ask until the player enters a number.
    print('How many snails shall race? Max:', MAX_NUM_SNAILS)
    response = input('> ')
...
snailNames = []  # List of string for snail names.
for i in range(1, numSnailsRacing + 1):
    while True:  #  ask until  player enters valid name
        ...
        if len(name) == 0:
            ...
            break  #  entered name is acceptable.
...
time.sleep(1)  #  pause right before the race starts.
...
while True:  # Main program loop.
    ...
    time.sleep(0.4)  # the lower this value, the faster the snails 
